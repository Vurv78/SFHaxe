...
/*
    Tries to get the return type from a function data struct's docstring.
    If it starts with a whitelisted type, set to that.
*/
function getReturnType(fn_data:FnData):String {
    // Since Starfall doesn't have good / consistent docs, we will only have this be typed in whether a function returns something.
    trace(fn_data.returns);
...
function genLibrary( lib_name:String, class_data: DocData ) {
    ...
    PairTools.pairsEach( class_data.methods, function(fn_name, fn_data) {
        ...
        if (realm != "SHARED") {
            // CLIENT or SERVER
            haxePush('\t#if $realm @:native("$fn_name") public static function ${mangleName(fn_name)}(${ opt_unwrap_or( getArgs(fn_data), "" ) }):${ getReturnType(fn_data) };#end\n');
        }else{
            haxePush('\t@:native("$fn_name") public static function ${mangleName(fn_name)}(${ opt_unwrap_or( getArgs(fn_data), "" ) }):${ getReturnType(fn_data) };\n');
        }
    });
    haxePush("}\n\n");
    ...
    haxePush('\nextern class ${type_name}Data {\n');
        PairTools.pairsEach( class_data.methods, function(fn_name, fn_data) {
            ...
            if (op_data != null) {
                ...
                if (realm != "SHARED") {
                    // CLIENT or SERVER
                    haxePush('\t#if $realm @:native("$fn_name") public function ${mangleName(fn_name)}(${ opt_unwrap_or( getArgs(fn_data), "") }):${ getReturnType(fn_data) };#end\n');
                }else{
                    haxePush('\t@:native("$fn_name") public function ${mangleName(fn_name)}(${ opt_unwrap_or( getArgs(fn_data), "") }):${ getReturnType(fn_data) };\n');
                }
            }
        });
    haxePush("}\n\n");
